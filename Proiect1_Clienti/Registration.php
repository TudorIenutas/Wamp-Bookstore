...
// Memoram rezultatul, astfel incat sa putem verifica daca contulexista in baza de date.
    ...
    echo 'Username existent, alegeti altul!';
        ...
        if ($stmt = $con->prepare('INSERT INTO clienti (client_username, client_pass, client_email, client_str, client_oras, client_tara, client_codpost, client_nrcard, client_tipcard, client_dataexp, acceptareemail, client_nume) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')) {
        ...
        $stmt->bind_param('sssssssissis', $_POST['username'], $password, $_POST['email'], $_POST['str'], $_POST['oras'],$_POST['tara'], $_POST['codpostal'], $_POST['nrcard'], $_POST['tipcard'],$_POST['dataexp'], $_POST['acceptareemail'],$_POST['nume']);
        $stmt->execute();
        ...
        header('Location: index.html');
            ...
            // Ceva nu este in regula cu declaratia sql, verificati pentru a va asigura ca tabelul conturilor 
            //exista cu toate cele 3 campuri.
            echo 'Nu se poate face prepare statement2!';
            }
            $con->close();
            ?>
